...
using Predictor = romea::core::localisation::R2HKFPredictor;
...
TEST(TestR2HKFPredictor, zeroMotionKeepsRelativePosition)
{
...
TEST(TestR2HKFPredictor, followerForwardMotionPropagatesRelativePosition)
{
  auto predictor = make_predictor();
  auto previous = make_state();
  previous.state.X() << 4.0, 2.0;
...
TEST(TestR2HKFPredictor, staticStateResetsWhenAnyProprioceptiveDataIsLost)
{
