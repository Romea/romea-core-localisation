  ...
  AddOn & currentAddon)
{
  ...
  const double & iz = tagAntennaPosition.z() + currentObservation.terrain_elevation;

  const double & rx = currentObservation.responder_position.x();
  const double & ry = currentObservation.responder_position.y();
  const double & rz = currentObservation.responder_position.z();
  ...
  // log
  if (log_file_.is_open()) {
    log_file_ << propagatedState_.Y() << " ";
    log_file_ << propagatedState_.R() << " ";
