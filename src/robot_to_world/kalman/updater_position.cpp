...
//--------------------------------------------------------------------------
void R2WKFUpdaterPosition::update(const Duration& duration,
    ...
    case FSMState::RUNNING:
      if (trigger_mode_ == trigger_mode::ALWAYS) {
        try {
          ...
          std::cout
              << " FSM : POSITION UPDATE HAS FAILED, RESET AND GO TO INIT MODE"
              << std::endl;
...
//-----------------------------------------------------------------------------
void R2WKFUpdaterPosition::update_(const Duration& duration,
...
//-----------------------------------------------------------------------------
bool R2WKFUpdaterPosition::set_(const Duration& duration,
                                const Observation& current_observation,
  ...
  if (!std::isnan(current_input.U(MetaState::LINEAR_SPEED_X_BODY)) &&
    ...
    current_state.X().segment<2>(MetaState::POSITION_X) =
        current_observation.Y();

    current_state.P().block<2, 2>(
        MetaState::POSITION_X, MetaState::POSITION_X) = current_observation.R();
