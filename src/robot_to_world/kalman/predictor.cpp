...
//--------------------------------------------------------------------------
void R2WKFPredictor::predict_(const MetaState& previous_meta_state,
...
//-----------------------------------------------------------------------------
void R2WKFPredictor::predictState_(const State& previous_state,
  ...
  current_state.X(MetaState::POSITION_X) =
      x_ + vx_ * dT_cos_theta_wdT_ - vy_ * dT_sin_theta_wdT_;
  current_state.X(MetaState::POSITION_Y) =
      y_ + vx_ * dT_sin_theta_wdT_ + vy_ * dT_cos_theta_wdT_;
  ...
  jG_(MetaState::POSITION_X, MetaState::LINEAR_SPEED_X_BODY) =
      dT_cos_theta_wdT_;
  jG_(MetaState::POSITION_Y, MetaState::LINEAR_SPEED_X_BODY) =
      dT_sin_theta_wdT_;
  jG_(MetaState::POSITION_X, MetaState::LINEAR_SPEED_Y_BODY) =
      -dT_sin_theta_wdT_;
  jG_(MetaState::POSITION_Y, MetaState::LINEAR_SPEED_Y_BODY) =
      dT_cos_theta_wdT_;
...
//-----------------------------------------------------------------------------
void R2WKFPredictor::predictAddOn_(const AddOn& previous_add_on,
...
//-----------------------------------------------------------------------------
bool R2WKFPredictor::stop_(const Duration& duration,
  ...
  if (positionCircularErrorProbability >
