//romea
#include "romea_localisation/robot_to_world/particle/R2WLocalisationPFUpdaterPose.hpp"
...
//-----------------------------------------------------------------------------
void R2WLocalisationPFUpdaterPose::computeInnovation_(const PFGaussianUpdaterCore::Observation &observation,
                                                      const RawMajorVector & weights)
{
  double weightSum = weights.sum();

  aprioriObservation_.Y(MetaState::POSITION_X) = (aprioriObservations_.row(MetaState::POSITION_X)*weights).sum()/weightSum;
  aprioriObservation_.Y(MetaState::POSITION_Y) = (aprioriObservations_.row(MetaState::POSITION_Y)*weights).sum()/weightSum;
  aprioriObservation_.Y(MetaState::ORIENTATION_Z) = std::atan2((aprioriObservations_.row(MetaState::ORIENTATION_Z).sin()*weights).sum()/weightSum,
  ...
  {
    aprioriMeanCenteredObservations_(MetaState::ORIENTATION_Z,n)= betweenMinusPiAndPi(aprioriMeanCenteredObservations_(MetaState::ORIENTATION_Z,n));
  }
                                        ...
                                        AddOn &currentAddon)
{
     ...
     !std::isnan(currentInput.U(MetaState::ANGULAR_SPEED_Z_BODY)))
  {
    ...
    levelArmCompensation_.compute(currentAddon.roll,
                                  ...
                                  currentObservation.R(ObservationPose::ORIENTATION_Z,ObservationPose::ORIENTATION_Z),
                                  currentObservation.levelArm);
