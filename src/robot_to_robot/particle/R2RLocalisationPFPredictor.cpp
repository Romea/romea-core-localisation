...
//--------------------------------------------------------------------------
void R2RLocalisationPFPredictor::predict_(const MetaState &previousMetaState,
...
void  R2RLocalisationPFPredictor::drawFollowerInputs(const Input & previousInput)
{
  ...
  QUf_ = previousInput.QU().block<3,3>(MetaState::LINEAR_SPEED_X_BODY,
...
void  R2RLocalisationPFPredictor::drawLeaderInputs(const Input & previousInput)
{
  ...
  QUl_ = previousInput.QU().block<3,3>(MetaState::LEADER_LINEAR_SPEED_X_BODY,
...
//-----------------------------------------------------------------------------
void R2RLocalisationPFPredictor::predictState_(const State &previousState,
...
//-----------------------------------------------------------------------------
void R2RLocalisationPFPredictor::predictAddOn_(const AddOn & previousAddOn,
                                               ...
                                               AddOn &currentAddOn)
{
     ...
     currentAddOn.robotTrajectory[0].norm()>0.1)
  {
    ...
    double x = (currentState.particles.row(MetaState::LEADER_POSITION_X)*currentState.weights).sum();
    double y = (currentState.particles.row(MetaState::LEADER_POSITION_Y)*currentState.weights).sum();
...
//-----------------------------------------------------------------------------
bool R2RLocalisationPFPredictor::stop_(const Duration & duration,
                                       const MetaState &meatastate)
{
  Duration durationInDeadReckoningMode = duration-meatastate.addon.lastExteroceptiveUpdate.time;

  double travelledDistanceInDeadReckoningMode = meatastate.addon.travelledDistance-meatastate.addon.lastExteroceptiveUpdate.travelledDistance;
