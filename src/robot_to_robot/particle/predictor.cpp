...
//--------------------------------------------------------------------------
void R2RPFPredictor::predict_(const MetaState& previous_meta_state,
...
void R2RPFPredictor::drawFollowerInputs(const Input& previous_input) {
  ...
  QUf_ = previous_input.QU().block<3, 3>(MetaState::LINEAR_SPEED_X_BODY,
...
void R2RPFPredictor::drawLeaderInputs(const Input& previous_input) {
  Ul_ = previous_input.U().segment<3>(MetaState::LEADER_LINEAR_SPEED_X_BODY) *
        dt_;

  QUl_ =
      previous_input.QU().block<3, 3>(MetaState::LEADER_LINEAR_SPEED_X_BODY,
...
//-----------------------------------------------------------------------------
void R2RPFPredictor::predictState_(const State& previous_state,
  ...
  cos_courses_ =
      current_state.particles.row(MetaState::LEADER_ORIENTATION_Z).cos();
  sin_courses_ =
      current_state.particles.row(MetaState::LEADER_ORIENTATION_Z).sin();
  ...
  current_state.particles.row(MetaState::LEADER_POSITION_Y) =
      ...
      sin_courses_ * randomUl_.row(MetaState::LINEAR_SPEED_X_BODY) *
          cos_courses_ * randomUl_.row(MetaState::LINEAR_SPEED_Y_BODY);

  current_state.weights = previous_state.weights;
}

//-----------------------------------------------------------------------------
void R2RPFPredictor::predictAddOn_(const AddOn& previous_add_on,
  ...
  if (current_add_on.robot_trajectory.size() == 0 ||
    ...
    double x = (current_state.particles.row(MetaState::LEADER_POSITION_X) *
                current_state.weights)
                   .sum();
    double y = (current_state.particles.row(MetaState::LEADER_POSITION_Y) *
                current_state.weights)
                   .sum();
...
//-----------------------------------------------------------------------------
bool R2RPFPredictor::stop_(const Duration& duration,
                           const MetaState& meatastate) {
  Duration durationInDeadReckoningMode =
      duration - meatastate.addon.last_exteroceptive_update.time;
