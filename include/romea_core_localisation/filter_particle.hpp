...
#ifndef ROMEA_CORE_LOCALISATION__FILTER_PARTICLE_HPP_
#define ROMEA_CORE_LOCALISATION__FILTER_PARTICLE_HPP_
...
public:
  ...
  Filter(
    ...
    PredictorArgs &&... predictor_args)
  : impl_(
      std::make_unique<typename Base::Filter>(state_pool_size, number_of_particles),
      std::make_unique<typename Base::Predictor>(
        number_of_particles, std::forward<PredictorArgs>(predictor_args)...),
    ...
    Args &&... args)
  {
    auto updater = std::make_unique<Updater>(
      updater_name, minimal_rate, std::forward<Args>(args)...);
...
#endif  // ROMEA_CORE_LOCALISATION__FILTER_PARTICLE_HPP_
