...
#ifndef ROMEA_CORE_LOCALISATION__UPDATER_LINEAR_SPEEDS_HPP_
#define ROMEA_CORE_LOCALISATION__UPDATER_LINEAR_SPEEDS_HPP_
...
class UpdaterLinearSpeeds : public UpdaterProprioceptive
{
public:
  using Observation = ObservationLinearSpeeds;

  UpdaterLinearSpeeds(const std::string & updater_name, const double & minimal_rate)
...
private:
  ...
  {
    input.U().template segment<2>(MetaState::LINEAR_SPEED_X_BODY) = current_observation.Y();

    input.QU().template block<2, 2>(
...
#endif  // ROMEA_CORE_LOCALISATION__UPDATER_LINEAR_SPEEDS_HPP_
